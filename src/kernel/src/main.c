...
int main(int argc, char* argv[]) {
    ...
    t_log* logger = log_create("log_del_kernel.log", "Kernel", 1, LOG_LEVEL_DEBUG);
	...
	while (1) {
		...
		switch (cod_op) {
		case MENSAJE:
			decir_hola("COD. MENSAJE RECIBIDO");
		...
		case PAQUETE:
			...
			break;
		case -1:
			decir_hola("el cliente se desconecto. Terminando servidor");
			log_destroy(logger);
			return EXIT_FAILURE;
		default:
